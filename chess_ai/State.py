...
class State(object):
  ...
  def serialize(self) -> torch.Tensor:
    ...
    for i in range(64):
      ...
      if pp is not None:
        raw_pieces_state[i] = {"P": 1, "N": 2, "B": 3, "R": 4, "Q": 5, "K": 6, \
                     "p": 7, "n":8, "b":9, "r":10, "q":11, "k": 12}[pp.symbol()]
    ...
    if self.board.ep_square is not None:
        en_passant_state = one_hot(torch.tensor(self.board.ep_square, dtype=torch.int64), num_classes=64)
